...
class MchController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            'total_in_queue' => PatientVisit::where('department_id', 7)
                ->where('status', 1)
                ->count(),
            'pending_assignment' => PatientVisit::where('department_id', 7)
                ->where('status', 1)
                ->whereNull('to_queue')
                ->count(),
            'assigned_today' => PatientVisit::where('department_id', 7)
                ->whereDate('created_at', today())
                ->where('status', 1)
                ->count(),
        ];
        return Inertia::render('mch/index',[
