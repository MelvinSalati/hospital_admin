...
class TheaterController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            'total_in_queue' => PatientVisit::where('department_id', 4)
                ->where('status', 1)
                ->count(),
            'pending_assignment' => PatientVisit::where('department_id', 4)
                ->where('status', 1)
                ->whereNull('to_queue')
                ->count(),
            'assigned_today' => PatientVisit::where('department_id', 4)
