...
export function ReusableTable<T extends Record<string, any>>({
  ...
  const [orderBy, setOrderBy] = useState<string | keyof T | undefined>(defaultOrderBy);
  ...
  const filteredData = useMemo(() => {
    ...
    if (statusFilter && statusFilterKey) {
      result = result.filter((row) => {
        ...
        return status != null && String(status) === String(statusFilter);
      });
    }
    ...
    return result;
  }, [data, columns, globalFilter, statusFilter, statusFilterKey, orderBy, order]);
  ...
  const handleStatusFilterChange = useCallback((e: SelectChangeEvent<string | number>) => {
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 3 }}>
      {/* Header */}
      <FilterBar>
        {title && <TitleTypography variant="h6">{title}</TitleTypography>}
